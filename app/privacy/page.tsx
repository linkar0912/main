...
export default function PrivacyPage() {
  ...
  return (
    <PublicPage currentPath="/privacy" title="Privacy policy" intro={`${PRODUCT_NAME} helps creators and businesses automate Instagram conversations and public Facebook Page comment replies. This policy explains what we collect, why we use it, and how you can ask us to delete it.`}>
      ...
      <p>We use Google Analytics to understand how the site and app are used, on both our public pages and the signed-in application. Google sets cookies in your browser that distinguish one visitor from another across pages and visits. Google receives the page you viewed, the page title, the referring page, your device and browser details, and an approximate location that Google derives from your IP address.</p>
      <p>Page addresses are shortened before they are sent. Where an address contains an identifier, such as a deletion request code, a workspace, or an automation, that part is replaced with a placeholder, and query strings and fragments are removed entirely. Google therefore receives the shape of the page you visited and not the identifiers in it. We do not send your name, email address, workspace, or connected account details to Google, and we do not link analytics activity to your {PRODUCT_NAME} account.</p>
      <p>You can refuse or delete these cookies in your browser settings, or install Google&#8217;s opt-out browser add-on. Refusing them does not affect any automation you have configured. Analytics runs only when {PRODUCT_NAME} is configured with a measurement identifier, so it may be absent in some deployments.</p>
      <h2>Sharing and processors</h2>
      <p>We share information only with service providers needed to host the application, with Google for the site analytics described above, and with Meta’s APIs when you ask {PRODUCT_NAME} to send or receive Instagram or Facebook Page data. We do not sell personal information or use connected social content for advertising.</p>
