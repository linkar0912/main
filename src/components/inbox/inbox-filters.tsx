...
export function InboxFilters({ value, labels, onChange }: { value: InboxFiltersValue; labels: string[]; onChange: (value: InboxFiltersValue) => void }) {
  ...
  return <div className="conversation-filter-stack">
    ...
    <div className="conversation-primary-filters">
      ...
      <button type="button" className="conversation-more-filters" aria-expanded={expanded} aria-controls="conversation-advanced-filters" onClick={() => setExpanded((current) => !current)}><SlidersHorizontal size={15} aria-hidden="true" />{expanded ? "Hide filters" : "More filters"}</button>
    </div>
    {expanded && <div className="conversation-advanced-filters" id="conversation-advanced-filters">
