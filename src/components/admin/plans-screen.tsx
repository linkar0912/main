...
function PremiumInviteManager({ inviteCodes, onError }: { inviteCodes: InviteCode[]; onError: (message: string | null) => void }) {
  ...
  async function create(event: FormEvent) {
    ...
    try {
      const response = await fetch("/api/admin/invite-codes", { method: "POST", headers: { "content-type": "application/json", "x-admin-reason": reason, "idempotency-key": `admin-${crypto.randomUUID()}` }, body: JSON.stringify({ label, expiresAt: expiresAt ? new Date(expiresAt).toISOString() : null }) });
      const payload = await response.json() as { data?: { code?: string }; error?: string };
      if (!response.ok || !payload.data?.code) throw new Error(payload.error ?? "invite_code_create_failed");
      setCreatedCode(payload.data.code); setLabel(""); setExpiresAt(""); setReason(""); router.refresh();
...
export function PlansScreen({ plans, inviteCodes = [] }: { plans: Plan[]; inviteCodes?: InviteCode[] }) {
  ...
  return <main className="page-wrap admin-resource-page"><header className="page-header"><div><p className="eyebrow">Linkar operator / entitlements</p><h1>Plans and limits</h1><p className="muted page-lede">Define enforceable templates. Empty limits mean unlimited; retired plans remain attached to existing workspaces.</p></div><span className="admin-count-badge"><WalletCards size={16} /> {plans.length} templates</span></header>{error ? <div className="form-error" role="alert">{error}</div> : null}<PremiumInviteManager inviteCodes={inviteCodes} onError={setError} /><form className="panel admin-plan-card admin-new-plan" onSubmit={create}><div className="panel-heading"><div><p className="eyebrow">New template</p><h2>Create plan</h2></div><Plus size={20} /></div><label className="field"><span>Stable key</span><input required pattern="[a-z][a-z0-9_-]{1,39}" value={key} onChange={(event) => setKey(event.target.value)} placeholder="growth" /></label><PlanFields value={value} onChange={setValue} /><label className="field"><span>Operator reason</span><input required minLength={3} value={reason} onChange={(event) => setReason(event.target.value)} /></label><button className="button button-primary" disabled={creating} type="submit"><Plus size={16} /> Create plan</button></form><section className="admin-plan-stack" aria-label="Plan templates">{plans.map((plan) => <PlanEditor key={plan.id} plan={plan} onError={setError} />)}</section></main>;
}
