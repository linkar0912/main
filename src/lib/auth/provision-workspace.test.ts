...
describe("provisionWorkspace", () => {
  it("creates a fresh workspace owned by the email when there is no invitation", async () => {
    ...
    const workspaceId = await provisionWorkspace({ email: "owner@example.com", invitation: null, repository });
  ...
  it("joins the invitation's workspace and marks it accepted, without creating a new workspace", async () => {
    ...
    const workspaceId = await provisionWorkspace({ email: "invited@example.com", invitation, repository });
